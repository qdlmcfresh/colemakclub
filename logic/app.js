...
var customLayoutMap = localStorage.getItem('customKeyboard');
...
saveButton.addEventListener('click', () => {
	customInput.style.transform = 'scaleX(0)';
	localStorage.setItem('customKeyboard', JSON.stringify(layoutMaps['custom']));
	localStorage.setItem('customLevelsDictionary', JSON.stringify(levelDictionaries['custom']));
	// remove active class from current key
	clearSelectedInput();
	init();
});
